...
export class TokenRefreshScheduler {
	...
	start(): void {
		...
		this.logger.info(
			`Starting token refresh scheduler (check every ${this.checkInterval / 1000}s, refresh threshold ${this.refreshThreshold / 1000}s)`,
		);
		this.isRunning = true;
	...
	private async checkAndRefreshTokens(): Promise<void> {
		try {
				...
				for (const [projectId, projectCapabilities] of capabilities) {
					...
					for (const server of projectCapabilities.servers) {
						...
						if (timeUntilExpiry < this.refreshThreshold) {
							...
							this.logger.info(
								`Refreshing MCP token for ${projectId}/${serverId} (expires in ${expiryMinutes} minutes)`,
							);
							...
							try {
								...
								if (success) {
									refreshedCount++;
									this.logger.success(
										`Successfully refreshed MCP token for ${projectId}/${serverId}`,
									);
								} else {
									failedCount++;
									this.logger.failure(
										`Failed to refresh MCP token for ${projectId}/${serverId}`,
									);
								}
							} catch (error: any) {
								failedCount++;
								this.logger.failure(
									`Error refreshing MCP token for ${projectId}/${serverId}: ${error.message}`,
								);
							}
						}
					}
			...
			if (this.gitIntegrationManager) {
				...
				for (const integration of gitIntegrations) {
					...
					if (timeUntilExpiry < this.refreshThreshold) {
						...
						this.logger.info(
							`Refreshing Git integration token for ${integration.platform} (expires in ${expiryMinutes} minutes)`,
						);

						try {
							const success =
								await this.gitIntegrationManager.refreshAccessToken(
							...
							if (success) {
								refreshedCount++;
								this.logger.success(
									`Successfully refreshed Git integration token for ${integration.platform}`,
								);
							} else {
								failedCount++;
								this.logger.failure(
									`Failed to refresh Git integration token for ${integration.platform}`,
								);
							}
						} catch (error: any) {
							failedCount++;
							this.logger.failure(
								`Error refreshing Git integration token for ${integration.platform}: ${error.message}`,
							);
						}
					}
				}
			...
			if (refreshedCount > 0 || failedCount > 0) {
				this.logger.info(
					`Token refresh check: ${checkedCount} checked, ${refreshedCount} refreshed, ${failedCount} failed`,
				);
			}
		} catch (error: any) {
