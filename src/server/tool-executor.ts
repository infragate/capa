...
import type { CommandSpec, ToolCommandDefinition } from "../types/capabilities";
...
export class CommandToolExecutor {
	...
	async execute(
		...
		if (definition.init && (!initState || !initState.initialized)) {
			...
			if (!initResult.success) {
				...
				// Store error
				this.db.setToolInitialized(
					this.projectId,
					toolId,
					initResult.error || "Init failed",
				);
				return {
					success: false,
			...
			this.logger.failure(
				`Tool initialization previously failed: ${initState.last_error}`,
			);
			return {
				success: false,
	...
	private async runCommand(
		...
		const resolvedSpec = resolveVariablesInObject(
			spec,
			this.projectId,
			this.db,
		);
		...
		const substituted = templateTokens.map((t) =>
		...
		const cwd = spec.dir
			? resolve(this.projectPath, spec.dir)
			: this.projectPath;

		this.logger.info(
			`          Running command: ${program} ${programArgs.join(" ")}`,
		);
