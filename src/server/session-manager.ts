...
export class SessionManager {
	...
	createSession(projectId: string): SessionInfo {
		...
		this.logger.info(
			`Creating session: ${sessionId} for project: ${projectId}`,
		);
	...
	setupTools(sessionId: string, skillIds: string[]): string[] {
		...
		if (!capabilities) {
			throw new Error(
				`No capabilities configured for project: ${session.projectId}`,
			);
		}
	...
	getProjectCapabilities(projectId: string): Capabilities | null {
		...
		const promise = Promise.resolve(
			this.loadCapabilitiesFromDb(projectId),
		).finally(() => this.capabilitiesLoadInflight.delete(projectId));
	...
	getToolDefinition(projectId: string, qualifiedName: string): Tool | null {
		...
		const exact = capabilities.tools.find(
	...
	private cleanupExpiredSessions(): void {
		...
		try {
			...
			this.logger.debug(
				`Skipping session cleanup: ${(error as Error).message}`,
			);
			return;
		}
