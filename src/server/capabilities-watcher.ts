...
export type CapabilitiesChangeHandler = (projectId: string) => void | Promise<void>;
...
export class CapabilitiesFileWatcher {
  ...
  async watchProject(projectId: string, projectPath: string): Promise<void> {
    ...
    if (this.projectPaths.get(projectId) === projectPath && this.watchers.has(projectId)) {
      return;
    }
    ...
    try {
      ...
      this.log.warn(`Capabilities watch skipped for ${projectId}: ${err?.message ?? err}`);
      return;
    }
    if (!file) {
    ...
    try {
      const w = watch(capsDir, { persistent: true }, (_event, filename) => {
        if (filename && filename !== capsName && basename(String(filename)) !== capsName) {
          return;
        }
        this.schedule(projectId);
      });
      w.on('error', (err) => {
        this.log.warn(`Capabilities watch error for ${projectId}: ${err.message}`);
      });
  ...
  private async fire(projectId: string): Promise<void> {
    ...
    try {
      ...
      this.log.warn(`Failed to reload capabilities for ${projectId}: ${err?.message ?? err}`);
    } finally {
      this.running.delete(projectId);
