...
function pluginsCacheKey(plugins: NonNullable<Capabilities['plugins']>): string {
  return plugins
    .map((p) => `${p.id ?? ''}|${p.type}|${p.def?.repo ?? ''}|${p.def?.version ?? ''}|${p.def?.ref ?? ''}`)
    .join('||');
}
...
export async function resolveEffectiveCapabilities(
  ...
  try {
    const { mergedCapabilities, tempDirsToCleanup, warnings } = await resolvePlugins(
      capsForResolve,
      projectPath,
      projectId,
      authFetch,
...
export async function loadEffectiveCapabilities(
  ...
  if (cached && cached.mtimeMs === mtimeMs && cached.pluginsKey === pluginsKey) {
    return cached.caps;
  }
...
export function preserveDiscoveredOAuth2(
  ...
  for (const server of fresh.servers) {
    ...
    if (!prevAuth && !prevToken && !prevOAuth.resourceServer && !prevOAuth.registrationEndpoint) {
      continue;
    }

    const nextOAuth = { ...(server.def.oauth2 ?? {}) } as Record<string, unknown>;
    const nextAuth =
