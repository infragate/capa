...
export class MCPSubprocessesRepo {
	...
	upsert(
		subprocess: Omit<MCPSubprocess, "started_at" | "last_health_check"> &
			Partial<Pick<MCPSubprocess, "started_at" | "last_health_check">>,
	): void {
		const now = Date.now();
		const existing = this.db
			.query("SELECT id FROM mcp_subprocesses WHERE id = ?")
			.get(subprocess.id);
	...
	get(id: string): MCPSubprocess | null {
		return this.db
			.query("SELECT * FROM mcp_subprocesses WHERE id = ?")
			.get(id) as MCPSubprocess | null;
	}

	getByHash(hash: string): MCPSubprocess | null {
		return this.db
			.query("SELECT * FROM mcp_subprocesses WHERE config_hash = ?")
			.get(hash) as MCPSubprocess | null;
	}

	getAll(): MCPSubprocess[] {
		return this.db
			.query("SELECT * FROM mcp_subprocesses")
			.all() as MCPSubprocess[];
	}
