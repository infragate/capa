...
export async function materializeSnapshot(
	...
	mkdirSync(join(getRepoCacheDir(platform, repoPath), "snapshots"), {
		recursive: true,
	});
...
export async function getOrCreateSnapshot(
	...
	if (opts.pinnedSha && !noCache) {
		const fullSha = opts.pinnedSha.length === 40 ? opts.pinnedSha : null;
		if (fullSha) {
	...
	const mirrorDir = await ensureMirrorClone(
		platform,
		repoPath,
		authFetch,
		opts.repoUrl,
