...
function validateLockfile(parsed: unknown): Lockfile {
	...
	if (obj.version !== 1) {
		throw new Error(
			`Unsupported lockfile version: ${String(obj.version)} (expected 1)`,
		);
	}
	...
	return {
		version: 1,
		generator:
			typeof obj.generator === "string" ? obj.generator : `capa@${VERSION}`,
		generatedAt:
...
export class LockfileBuilder {
	...
	findSkill(
		...
		if ((entry.requestedVersion ?? null) !== (requestedVersion ?? null))
			return null;
	...
	findPlugin(query: {
		...
		for (const entry of this.plugins.values()) {
			...
			if (
				wantedSearch === null &&
				(entry.subpath ?? null) !== (query.subpath ?? null)
			)
				continue;
	...
	build(): Lockfile {
		const skills = [...this.skills.values()].sort((a, b) =>
