...
export function splitTreeRefAndSubpath(treePath: string): {
	refOrBranch: string;
	subpath?: string;
} {
	...
	const pluginRootIndex = segments.findIndex(
		(segment) => segment === "plugins",
	);
...
export function parseGithubRawUrl(
	url: string,
	...
	if (host === "github.com") {
		...
		if (rawIdx === -1 || rawIdx < 2 || segments.length < rawIdx + 3)
			return null;
...
export function parseGitlabRawUrl(
	url: string,
): { owner: string; repo: string; ref: string; path: string } | null {
	let parsed: URL;
	try {
