 ...
 */
function splitRepoString(raw: string): {
	repoPath: string;
	subpath?: string;
	search?: string;
	error?: string;
...
export function validatePluginDef(
	plugin: Plugin,
	...
	const {
		...
		search: repoSearch,
		error: repoError,
	} = splitRepoString(def.repo);
	if (repoError) {
		return { error: repoError };
	}
	if (!isValidPath(repoPath, 2)) {
		return {
			error: `Invalid repo path: "${def.repo}". Must be at least two segments (e.g. "owner/repo").`,
		};
	}

	if (type === "github" && repoPath.split("/").length !== 2) {
		return {
			error: `GitHub repo path must be exactly "owner/repo"; got "${def.repo}".`,
		};
	}
	...
	if (repoSearch !== undefined) {
		if (repoSearch.length === 0) {
			return {
				error: `Invalid search target in "${def.repo}": empty plugin name after "@".`,
			};
		}
 ...
 */
export function splitOwnerRepo(repoPath: string): {
	owner: string;
	repo: string;
} {
