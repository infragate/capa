...
- `search` (what `capa init` writes by default): only `search` and `call_tool` are exposed. The agent finds tools by keyword — `search('open a pull request')` — and calls what comes back. Discovery is by task rather than by skill id, which suits servers exposing many tools (`expose: all`), and skill `requires:` has no effect.
