...
- `search` (what `capa init` writes by default): only `search` and `call_tool` are exposed at startup; the agent finds tools by describing the task. Scales to servers that expose many tools.
